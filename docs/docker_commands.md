...
## 動作確認
まずは動作確認！Dockerを起動してから（Docker Desktopを起動してから）、ターミナル（端末／terminal）で下記のコマンドを実行してください。「Hello from Docker!」と戻ってくればOKです。
```
...
X Window Systemの動作を確認するため、パッケージ「x11-apps」をインストールし、アプリ「xeyes」を実行してみます。正常に実行できた場合は「マウスカーソルを見続ける目」が表示されます。<span style="color: #CC0066;">表示されない人は、設定などを間違えている可能性が高いので、ここまでの実験内容を再確認してください。</span>アプリを終了するときは「Ctrl＋c」を押します。なお、演習はこのままDockerコンテナーに入ったまま進めますが、抜けたい場合はコマンド`exit`を実行します。
```
$ sudo apt update
